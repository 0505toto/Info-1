/*==============================================
  ロゴアニメーション (ご提示のコード)
  jQueryとGSAP(TweenMax)ライブラリが必要です。
  (HTML側で読み込み済みです)
==============================================*/
...
function logoWebbab() {
  ...
  self.animation = function() {
    ...
    function rhombusRotation() { /* ... snip ... */ }
    function ellipsesAnimation() { /* ... snip ... */ }
    function circleIntersection() { /* ... snip ... */ }
    function textAnimation() { /* ... snip ... */ }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const draggables = document.querySelectorAll('.content-section');
    ...
    draggables.forEach(draggable => {
        ...
        draggable.addEventListener('dragstart', () => {
            draggable.classList.add('dragging'); // ドラッグ中の目印としてクラスを追加
        });

        // ドラッグ終了時
        draggable.addEventListener('dragend', () => {
            draggable.classList.remove('dragging'); // 目印のクラスを削除
        });
    });

    // ドロップされるコンテナの処理
    container.addEventListener('dragover', e => {
        e.preventDefault(); // デフォルトの動作を無効化しないとドロップできない
        
        const afterElement = getDragAfterElement(container, e.clientY);
        const dragging = document.querySelector('.dragging'); // ドラッグ中の要素を取得
        
        if (afterElement == null) {
            container.appendChild(dragging); // もしドロップ先がなければ末尾に追加
        } else {
            container.insertBefore(dragging, afterElement); // 特定の要素の前に挿入
        }
    });
    
    // マウスカーソルのY座標から、どの要素の前にドロップするかを判断する関数
    function getDragAfterElement(container, y) {
        // コンテナ内のドラッグ可能な要素をすべて取得（ドラッグ中のものを除く）
        const draggableElements = [...container.querySelectorAll('.content-section:not(.dragging)')];

        // 各要素の位置を調べ、カーソル位置に最も近い次の要素を見つける
        return draggableElements.reduce((closest, child) => {
            const box = child.getBoundingClientRect();
            const offset = y - box.top - box.height / 2; // 要素の中心からの距離
            
            // カーソルより下で、最も距離が近い要素を探す
