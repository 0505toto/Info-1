...
function logoWebbab() {
    ...
    self.setObj = function() {
        gsap.set(self.isoLogo, {attr:{rx: 50, ry: 50}, transformOrigin:"50% 50%", rotation: 90, scale: 0, autoAlpha: 1});
        gsap.set(self.logoCircle, {transformOrigin:"50% 50%", scale: 2, autoAlpha: 0});
        gsap.set(self.logoEllipse, {attr:{cx: 100, cy: 150, rx: 180, ry: 180}, strokeWidth: 60, transformOrigin:"50% 50%", autoAlpha: 0});
    };
    ...
    self.animation = function() {
        ...
        function rhombusRotation() {
            ...
            tl.to(self.isoLogo, { duration: 0.6, autoAlpha: 1, scale: 1, ease: "power1.in" })
              .to(self.isoLogo, { duration: 0.6, attr:{rx: 15, ry: 15}, rotation: -45, ease: "power1.out" });
            return tl;
        }
        
        function ellipsesAnimation() {
            var tl = gsap.timeline();
            tl.to(self.logoEllipse.eq(0), { duration: 0.4, attr:{ cx: 6, cy: -64, rx: 150, ry: 150 }, strokeWidth: 2, autoAlpha: 1, ease: "power2.out" })
              .to(self.logoEllipse.eq(1), { duration: 0.4, attr:{ cx: 138, cy: -80, rx: 150, ry: 150 }, strokeWidth: 2, autoAlpha: 1, ease: "power2.out" }, '-=0.2')
              .to(self.logoEllipse.eq(2), { duration: 0.4, attr:{ cx: 190, cy: 15, rx: 150, ry: 150 }, strokeWidth: 2, autoAlpha: 1, ease: "power2.out" }, '-=0.2');
            return tl;
        }
        
        function circleIntersection() {
            var tl = gsap.timeline();
            tl.fromTo(self.logoCircle, { autoAlpha: 0, scale: 0 }, { duration: 0.4, autoAlpha: 1, scale: 1, ease: "cubic.inOut", stagger: 0.1 });
            return tl;
        }
        
        function textAnimation() {
            var tl = gsap.timeline();
            tl.fromTo(self.wordLogo, 
                { autoAlpha: 0, scale: 0.8, x: -20, transformOrigin: "left center" }, 
                { duration: 1.5, autoAlpha:1, scale: 1, x: 0, ease: "elastic.out(1, 0.5)", stagger: 0.05 }
            );
            return tl;
        }
        
        // --- ★ここからが重要★ ---
        // MASTER SCENES & CALLBACK
        var master = gsap.timeline({
            // アニメーション完了後の処理を追加
            onComplete: function() {
                // ロゴを上に移動させ、コンテンツを表示する関数を呼び出す
                showMainContent();
            }
        });
...
$(document).ready(function() {
    ...
    // --- 1. オープニングアニメーションの実行 ---
    // $(window).load() は画像なども含めた全てが読み込まれてから実行する
    ...
    $('.section-header').on('click', function() {
        // クリックされたヘッダーの親(.draggable-section)を取得
    ...
    // マウスカーソルのY座標に基づいて、どこに要素を挿入すべきか判断する関数
