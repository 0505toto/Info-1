...
$(window).on('load', function() {
  ...
  function rhombusRotation() {
    ...
    tl.to($('#logo-webbab #iso'), 0.6, { autoAlpha: 1, scale: 1, ease: Power1.easeIn })
      .to($('#logo-webbab #iso'), 0.6, { attr: { rx: 15, ry: 15 }, rotation: -45, ease: Power1.easeOut });
    return tl;
  }

  function ellipsesAnimation() {
    const tl = new TimelineMax();
    tl.to($('.logo-ellipse').eq(0), 0.4, { attr: { cx: 6, cy: -64, rx: 150, ry: 150 }, strokeWidth: 2, autoAlpha: 1, ease: Power2.easeOut })
      .to($('.logo-ellipse').eq(1), 0.4, { attr: { cx: 138, cy: -80, rx: 150, ry: 150 }, strokeWidth: 2, autoAlpha: 1, ease: Power2.easeOut }, '-=0.2')
      .to($('.logo-ellipse').eq(2), 0.4, { attr: { cx: 190, cy: 15, rx: 150, ry: 150 }, strokeWidth: 2, autoAlpha: 1, ease: Power2.easeOut }, '-=0.2');
    return tl;
  }
  ...
  master.timeScale(0.9); // アニメーションの速度
});


// --- 2. ドラッグ＆ドロップ機能 ---
// ページのDOM（HTML構造）が読み込まれたらすぐに実行します。
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ドラッグした要素がドロップ先のエリア上にあるときの処理
    draggableContainer.addEventListener('dragover', e => {
        e.preventDefault(); // デフォルトの動作（ドロップを許可しない）を無効化
        
        const afterElement = getDragAfterElement(draggableContainer, e.clientY);
        
        if (afterElement == null) {
            // 他の要素の下にない場合は、コンテナの末尾に追加
    ...
    // マウスカーソルのY座標から、どの要素の前に挿入すべきかを判断する関数
    function getDragAfterElement(container, y) {
        // コンテナ内のドラッグ可能要素（ただし、現在ドラッグしている要素は除く）のリストを取得
